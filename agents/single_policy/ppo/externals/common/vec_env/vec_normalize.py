...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(self, venv, ob=False, ret=True, clipob=10., cliprew=10., gamma=0.99, epsilon=1e-8, use_tf=False,
                 obj_rms=False):
        VecEnvWrapper.__init__(self, venv)
        if use_tf:
            from agents.single_policy.ppo.externals.common.running_mean_std import TfRunningMeanStd
            ...
            from agents.single_policy.ppo.externals.common.running_mean_std import RunningMeanStd
    ...
    def step_wait(self):
        ...
        if self.obj_rms:
            ...
            for info in infos:
                info['obj'] = np.clip(info['obj'] / np.sqrt(self.obj_rms.var + self.epsilon), -self.cliprew,
                                      self.cliprew)
